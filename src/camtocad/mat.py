...
vakken staan kleine witte stippen: extra textuur voor het uitsnijden van silhouetten
(masks.py) zonder de detectie van schaakbordhoeken te hinderen (stippen blijven weg van
de hoeken).
...
class MatSpec:
    ...
    @property
    def mat_id(self) -> str:
        return f"CTC-{self.name}-{self.seed}"
...
PRESETS: dict[str, MatSpec] = {
    "A4": MatSpec("A4", 297.0, 210.0, 12, 8),
    "A3": MatSpec("A3", 420.0, 297.0, 18, 12),
}
...
def get_spec(spec: str | MatSpec | dict) -> MatSpec:
    ...
    try:
        ...
        raise ValueError(f"Onbekende mat '{spec}'; kies uit {', '.join(PRESETS)}") from None
...
def board_to_mat(points_board: np.ndarray, spec: MatSpec) -> np.ndarray:
    """Zet board-coördinaten (y omlaag, z het papier in) om naar mat-coördinaten (Y en Z omhoog)."""
    p = np.asarray(points_board, dtype=float).reshape(-1, 3)
    return np.column_stack([p[:, 0], spec.board_h_mm - p[:, 1], -p[:, 2]])


def board_to_mat_transform(spec: MatSpec) -> tuple[np.ndarray, np.ndarray]:
    """(A, a) zodat X_mat = A @ X_board + a."""
    return np.diag([1.0, -1.0, -1.0]), np.array([0.0, spec.board_h_mm, 0.0])
...
def board_rects(spec: MatSpec, dots: bool = True) -> list[Rect]:
    ...
    if dots and spec.dots_per_square > 0:
...
def page_rects(spec: MatSpec) -> tuple[list[Rect], list[tuple[float, float, str, float]]]:
    ...
    subtitle = (
        f"{spec.squares_x} x {spec.squares_y} vakken van {square} mm, markers {marker} mm ({spec.dictionary}). "
        "Print op 100% (werkelijke grootte), niet 'aanpassen aan pagina'. Gebruik mat papier en leg de mat vlak."
    )
    texts.append((ox, top - 9.0, f"Cam-to-CAD kalibratiemat {spec.name} - ID {spec.mat_id}", 10.0))
